 ...
 * changes underneath — which produced a genuinely nasty failure: the process held
 * the right value, a newly added route read it correctly, and the existing route
 * read empty, so the app dispatched no worker and transcribed nothing with no
 * error anywhere. `readFileSync` cannot be folded away.
 */
...
/**
 * One room per sandbox, so the two humans only need to be handed the same link.
 * `?room=` overrides it when several calls have to run side by side.
 */
export function defaultRoomName(): string {
  const name = sandboxConfig().name.trim();
  return name ? `assist-${name}` : 'assist-room';
}
...
export function assistConfig(room?: string): AssistConfig {
