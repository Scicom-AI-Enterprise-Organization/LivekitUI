...
- `sandbox.ts` — clones a template into `data/sandboxes/<name>`, writes its `.env.local` (including `NEXT_PUBLIC_AGENT_NAME`, which decides whether the sandbox requests a named agent dispatch), and runs it on a reused free port. It also writes `LIVEKIT_SERVER_URL` and `SANDBOX_NAME`: a template that calls the SDK from its own routes cannot use the browser-facing `LIVEKIT_URL` (under Docker the container may not resolve it), and one that wants a stable room per sandbox needs to know its own name. Four things about it are load-bearing:

  - **The spawn's `env` is what applies the sandbox's variables, not `.env.local`.** `@next/env` snapshots `process.env` on first load and then takes a key from an env file **only when it is absent from that snapshot** — an inherited variable is never overwritten. So the dashboard's own `LIVEKIT_URL` (an internal address: `http://livekit-server:7880` under Docker, a service name under Kubernetes) reached the child `next dev` through `{...process.env}` and silently won over the public URL written beside it, and the template handed that internal address to the browser as `serverUrl`. `sandboxEnv` is therefore spread **after** `process.env` in the `spawn` call; the file is written for the reader. Nothing about this reads as a misconfiguration — `LIVEKIT_PUBLIC_URL` was correct and the console (which goes through `runtime-config.ts`) worked, so only sandboxes broke. Behind TLS it never reaches the network either: an https page cannot open a `ws://` socket, so the browser blocks it as mixed content and reports a `SecurityError` about constructing a WebSocket rather than an unresolvable host. The same trap hides behind any variable the dashboard also defines — a name only the sandbox uses (`SANDBOX_NAME`, `AGENT_NAME`) was picked up from the file all along, which is why this surfaced on one variable and not the rest.
...
- `agent-assist-dual.ts` / `agent-assist-dual-config.ts` — the **dual-track** worker's deploy path, same split and the same rules (config half is Node-free and imported by the sandbox pages). Its config is `AssistWorkerConfig` plus `micRole`, so `AssistSettings` renders both forms and there is one normalizer per flavour rather than two half-duplicated ones; `micRole` has to be carried across `AssistSettings`'s `onChange` by hand, because that component hands back an `AssistWorkerConfig` and dropping the field would let the server's normalizer silently default it. Environment variables are prefixed `DUAL_` rather than `ASSIST_` — not to avoid a collision (each worker gets its own `.env.local`) but so a log line or a stray export says which worker it belongs to. **Both redeploy sweeps are template-filtered**, and that is not optional: the two templates store their worker under the same `assist` / `assistWorker` settings keys, so an unfiltered scan in either module would overwrite the other's `.env.local` with variables its Python reads none of — leaving a worker that starts, registers and transcribes nothing.
...
- `voice-sim.ts` — the same idea for a voice-agent sandbox, behind `POST /api/voice-sim`, sharing `assist-sim.ts`'s spawn, voice lookup and run log. `example/agent-voice-sim/simulate.py` joins as one ordinary participant and **takes turns from the agent's own `lk.agent.state`** rather than sleeping — talk over the agent and the run measures interruptions instead of the clean STT → EOU → TURN → LLM → TTS chain it exists to check. It publishes no metrics and no transcriptions of its own, so everything on the room's metric topic came from the agent; a simulator that reported its own STT would put two agents' numbers in one plot. Kept separate from the assist simulator because the calls are not the same shape — two humans and a worker that only listens, versus one caller and an agent that answers.
- `audio-input.ts` — the chunk an agent's audio arrives in (`AudioInputOptions.frame_size_ms`), shared by the builder and the assist form so the two offer the same sizes. **Node-free**, same rule as `providers.ts`. One chunk is one call into the noise filter and then into the VAD and STT, so the size is the filter's scheduling granularity and the buffering in front of everything else — it never changes how a filter processes (GTCRN is 256-sample hops through a 512-point window regardless). `normalizeAudioChunkMs` is an allow-list, not a range check, because the value is interpolated into generated Python and into a worker's environment.
